...
def generate_launch_description():
    ...
    # Use 'classic' as the fallback for real-world to satisfy the base Create3 model logic
    gazebo_arg = PythonExpression(["'ignition' if '", LaunchConfiguration('is_sim'), "' == 'true' else 'classic'"])

    robot_description_command = ParameterValue(
            Command([
                'xacro ',
                PathJoinSubstitution([
                    pkg_first_roomba, 'urdf', 'minchan_turtlebot4.urdf.xacro'
                ]),
                ' gazebo:=', gazebo_arg
            ]),
            value_type=str
    ...
    robot_state_publisher = Node(
        ...
        parameters=[{
            ...
            #'frame_prefix': 'minchan/'  # <--- ADD THIS LINE
        }]
    )
